...
class TableTest {
    ...
    @Test
    void placeAndRemoveCardFromTheTable() {

        //fiiling slots 1,2 with cards
        fillSomeSlots();
        

        //trying to remove the cards in slot 1
        table.removeCard(1);

        //Checking if the card was indeed removed from the table
        assertEquals(slotToCard[1], null);
    
        //placing card 6 on the same slot (1)
        table.placeCard(6,1);

        //checking if the card 6 was indeed placed on the table
        assertEquals(slotToCard[1], 6);
    ...
    @Test
    void CountCardsIsCorrect() {

        //filling the table with 2 cards
        fillSomeSlots();

        //checking countCards is correct
        assertEquals(table.countCards(), 2);

        //removing one cards from the table
        table.removeCard(1);
        
        //checking if COuntCards was decreased to 1
        assertEquals(table.countCards(), 1);
